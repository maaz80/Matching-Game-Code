...
export default function Game() {
    ...
    useEffect(() => {
        const shuffleName = Fruits.map(fruitItem => fruitItem.name).sort(() => Math.random() - 0.5);
        setRandomName(shuffleName);
    }, [Fruits]);
    ...
    function BackButton() {
        return (
            <Link to="/page4" className=" absolute top-0 left-0 mt-4 ml-4  cursor-pointer "><img className="md:w-24 md:h-24 w-16 h-16 hover:w-[100px] hover:h-[100px]" src="https://www.pngall.com/wp-content/uploads/12/Previous-Button-PNG-Images.png" alt="Back" /></Link>
        );
    }
                ...
                <div className="flex mt-12">
                    ...
                    {randomName.map((name, index) => (
                            <div key={index} className="flex flex-wrap">
                                <div className="blue md:h-52 md:w-40 h-24 w-[70px] bg-gradient-to-r from-blue-500 to-blue-300 border-4 rounded-md flex-wrap border-white shadow-lg shadow-black mt-10 flex justify-center items-center hover:shadow-md hover:border-blue-300" onClick={() => handleClickedName({ name })}>
                                    {(clickedCardName === name || matchedPairs.includes(name)) ? (
                                        <span className="poetsen-one-regular md:text-3xl text-sm drop-shadow-md drop-shadow-black">{name}</span>
